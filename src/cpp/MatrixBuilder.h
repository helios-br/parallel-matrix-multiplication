...
inline vector<vector<double>> convertToDoubleMatrix(vector<vector<int>> originalMatrix, int matrixOrder)
{
	vector<vector<double>> matrix(matrixOrder, vector<double>(matrixOrder));
	for (int i = 0; i < matrixOrder; i++)
		for (int j = 0; j < matrixOrder; j++)
